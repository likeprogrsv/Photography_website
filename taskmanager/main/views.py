...
def photos(request):
    query_tags = Photos.objects.annotate(
        tag=Func(F('category'), function='unnest')).values(
            'tag', 'id').order_by('tag').annotate(
                count=Count('id')).values('id', 'tag', 'count')
    tags = {}
...
def photography(request, photo_id):
    ...
    if photo is not None:
        return render(request, 'main/photography.html', {
            'photo': photo, 'photo_comments': photo_comments, 'form': form
            })
    else:
